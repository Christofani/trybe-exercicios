...
const averageAge = () => {
    const quantLivros = books.length;
    const somaLivros = books.reduce((sum, book) => {
        return sum + (book.releaseYear - book.author.birthYear);
    },0);
    return somaLivros / quantLivros; 
};
console.log(averageAge());


const longestNamedBook = () => books
    .reduce((mnome, book) => (mnome.name.length > book.name.length)? mnome: book);
